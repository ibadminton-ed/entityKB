...
can calculate the distance between any two entity representations to determine if they should be aligned (Section~\ref{prediction}).

	

	
    \subsection{Graph Representation Model\label{sec:model}}
...
%    Each input adjacency matrix describes the adjacency relationships among all nodes in the graph under each different relation.
 ...
 Our work improves \RGCNs in two ways. Firstly, we we use automatically learned semantic features such as entity names and attributes to
 help the \RGCN better capture the subtle correlations between two \KGs (Section~\ref{section:rgcn}.i).
...
Figure~\ref{all} demonstrates the overall architecture of our model. 	
