...
% described in Section~\ref{subsection:Node Representations} to construct $X$ to capture the semantic information. 	The second part is
 ...
 Like standard \GCNs, node relations are extracted from an adjacency matrix of neighboring nodes in a graph.
 Specifically, the adjacency matrices, $A=\{A_1,A_2,...,A_R |A_i \in \mathbb{R}^{N
 \times N} \}$, describe the adjacency relationships among $N$ nodes under $R$ different relations of graph $G$.
 To construct an adjacency matrix, we follow a number of steps. We first extract $R_0$ original
 relations from \KGs, then we add reverse relations in order to pass information from the opposite direction, before we insert the self
 loop to retain information of the node itself. \FIXME{ZW: What is a self loop? This is confusing.} These together compose $R=2R_0+1$ relations. 	In each layer $l$, the input is $X^{(l)} =
 \{x^{(l)}_1,x^{(l)}_2,...,x^{(l)}_{N} |x^{(l)}_{i} \in \mathbb{R}^{d^{(l)}}\}$. The forward propagation is formulated as:
 \FIXME{ZW: We need to refer to Figure~\ref{all} when discussing the inputs and layers.}


	\begin{equation}
	x_i^{(l+1)}=\mathrm{ReLU} (\sum\limits_{r \in R}\sum\limits_{j \in N_i^r}\frac{1}{|N_i^r|}W_r^{(l)}x_j^{(l)})
	\end{equation}
	As a departure from general \GCNs, our approach applies relation-specific transformations to an edge depending on the type and
direction of the edge. Later in the paper, we show that this strategy can better capture the multi-relational data characteristics of
real-world \KGs. Here $W_r^{(l)} \in \mathbb{R}^{d^{(l+1)} \times d^{(l)}}$ is the weight matrix of relation $r$; $N_i^r$ is the set of
neighbor indices of node $i$ under relation $r$, according to normalized adjacency matrix $\hat A_r$; and $\hat A_r$ is an approximate of
spectral convolutions on $A^r$, introduced by ~\cite{Kipf2016Semi}:
	\begin{equation}
	...
	\FIXME{We need to shrink this section if it is all about prior work.}
   ...
   alignment (Section~\ref{sec:motivation}). The node feature matrix is automatically constructed using the semantics of entity names and attribute information of
   entities.

	
	\eparagraph{Semantic initialization}
	\label{wordvector}
	Intuitively, the names of entities and their counterparts in multiple \KGs should be semantically similar. For example, \FIXME{refer to the motivation example?}
    Therefore, we leverage the pre-trained word embeddings to encode the semantic information for the names of entities.
	...
	\eparagraph{Attribute initialization}
